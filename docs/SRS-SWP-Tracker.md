...
Communication diagram nhấn mạnh **cấu trúc và quan hệ giữa các đối tượng** (structure and relationships between objects). Trên mỗi liên kết, **số thứ tự (1, 2, 3, ...)** thể hiện thứ tự gọi message trong kịch bản use case. Các đối tượng thường gồm: **View/Client** (giao diện), **Controller/Service** (tầng xử lý), **AppDbContext / API ngoài** (dữ liệu). Dưới đây cung cấp **đủ 11 Communication diagram** cho 11 use case.

**Cách đọc (sample):** Mũi tên `A -->|1: messageName| B` nghĩa là đối tượng A gửi message thứ nhất (1) tới B; số 2, 3... là các bước tiếp theo trong luồng.
...
|----|----------|-------------------------------|
| UC-01 | Đăng nhập / Đăng xuất | Login View, AccountController, UserManager, SignInManager |
...
| UC-10 | Đồng bộ GitHub | View, GitHubController/GitHubService, GitHub API, AppDbContext |
| UC-11 | Xem thống kê commit / báo cáo | Commits View, ReportController, AppDbContext |

---

**Communication Diagram 1 – UC-01 Đăng nhập / Đăng xuất**
...
**Communication Diagram 2 – UC-02 Quản lý nhóm (CRUD)**
...
**Communication Diagram 3 – UC-03 Quản lý giảng viên (CRUD lecturer)**
...
**Communication Diagram 6 – UC-06 Đồng bộ Jira (Sync issues → Tasks)**
...
**Communication Diagram 7 – UC-07 Quản lý công việc (Tạo task, phân công)**
...
**Communication Diagram 8 – UC-08 Cập nhật trạng thái task (Todo / InProgress / Done)**
...
**Communication Diagram 9 – UC-09 Tạo SRS (Generate & download)**
...
**Communication Diagram 10 – UC-10 Đồng bộ GitHub (Sync commits)**
...
*Trên đây là đủ 11 Communication diagram cho 11 use case (UC-01 đến UC-11).*
